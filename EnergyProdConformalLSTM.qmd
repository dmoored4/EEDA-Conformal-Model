...
function predict_daterange(df, first_prediction, last_prediction)
    ...
    results = [model(batched[first_pred_index])]
    #=
    for j in first_pred_index+1:length(batched)-1
        new_result = model([
            batched[j].past[1:1+length(weather_cols);
            last(results)])
...
```{julia}
predict_daterange(data, DateTime(2024,03,15,8,30), DateTime(2024,03,16,8,0))
```
